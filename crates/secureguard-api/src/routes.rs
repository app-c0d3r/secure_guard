...
use crate::{
    database::Database,
    handlers::{agent_communication, agents, auth, pipeline, threats},
};
...
fn threat_routes() -> Router<Database> {
    Router::new()
        ...
        .route(
            ...
            post(threats::create_security_event),
        )
        // Detection Rules
        ...
        .route(
            ...
            get(threats::analyze_threat_patterns),
        )
...
fn pipeline_routes() -> Router<Database> {
    Router::new()
        ...
        .route(
            "/emergency/isolate",
            post(pipeline::emergency_isolate_agents),
        )
        ...
        .route(
            ...
            get(pipeline::get_performance_analytics),
        )
        .route(
            "/analytics/threats",
            get(pipeline::get_threat_intelligence_summary),
        )
}
...
fn agent_communication_routes() -> Router<Database> {
    Router::new()
        ...
        .route(
            "/register",
            get(agent_communication::register_agent_enhanced),
        )
        // Agent Status and Monitoring
        .route("/status", get(agent_communication::list_agents_with_status))
        .route(
            "/status/:agent_id",
            get(agent_communication::get_agent_status_detailed),
        )
        .route(
            "/overview",
            get(agent_communication::get_communication_overview),
        )
        // Command Distribution
        .route(
            "/command/:agent_id",
            post(agent_communication::send_command_to_agent),
        )
        .route(
            "/command/bulk",
            post(agent_communication::send_bulk_command),
        )
        .route(
            "/command/broadcast",
            post(agent_communication::broadcast_emergency_command),
        )
        // Emergency Operations
        .route(
            "/emergency/isolate",
            post(agent_communication::emergency_isolate_agents),
        )
        // Performance and Analytics
        .route(
            "/metrics/:agent_id",
            get(agent_communication::get_agent_performance_metrics),
        )
}
