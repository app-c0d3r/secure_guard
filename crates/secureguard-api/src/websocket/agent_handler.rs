...
use secureguard_shared::{
    AgentMessage, DashboardMessage, CreateSecurityEventRequest, CommandResponse,
    AgentStatus, Severity
};
            ...
            while let Some(msg) = receiver.next().await {
                match msg {
                    ...
                    Ok(Message::Binary(data)) => {
                        debug!("Received binary data from agent {} ({} bytes)", agent_id, data.len());
...
async fn handle_agent_message(
    ...
    match message {
        AgentMessage::SecurityEvent(event_request) => {
            info!("Processing security event from agent {}: {}", agent_id, event_request.event_type);
            
            // Store the security event
            let event = threat_service.create_security_event(agent_id, event_request).await
            ...
            connection_manager.send_to_all_dashboards(&dashboard_message).await
                .map_err(|e| format!("Failed to notify dashboards: {}", e))?;
            
            // Check if this event generated any alerts
            let alerts = threat_service.get_alerts(Some(agent_id), None).await
                .map_err(|e| format!("Failed to check alerts: {}", e))?;
            
            // Notify about new alerts if any were created recently
            for alert in alerts.iter().take(1) { // Only notify about the most recent alert
                ...
                connection_manager.send_to_all_dashboards(&alert_message).await
        ...
        AgentMessage::CommandResponse { command_id, status, result } => {
            info!("Received command response from agent {}: {:?}", agent_id, status);
            
            // Update command status in database
            threat_service.update_command_status(
                command_id,
                status.clone(),
                result.clone()
            ).await
        ...
        AgentMessage::SystemMetrics { cpu_usage, memory_usage, disk_usage, network_connections, running_processes } => {
            debug!("Received system metrics from agent {}: CPU {}%, Memory {}%, Disk {}%", 
            ...
            agent_service.update_heartbeat(heartbeat).await
