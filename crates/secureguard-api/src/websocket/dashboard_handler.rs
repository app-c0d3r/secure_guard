...
use crate::{
    ...
    services::{
        agent_service::AgentService, auth_service::AuthService, threat_service::ThreatService,
        user_service::UserService,
    },
    websocket::connection_manager::ConnectionManager,
};
use secureguard_shared::{AgentStatus, DashboardMessage};
...
async fn handle_dashboard_socket(
        ...
        tokio::spawn(async move {
            while let Some(msg) = rx.recv().await {
                if let Err(e) = sender.send(Message::Text(msg)).await {
                    error!(
                        "Failed to send message to dashboard user {}: {}",
                        user_id, e
                    );
                    break;
                }
            ...
            while let Some(msg) = receiver.next().await {
                match msg {
                    Ok(Message::Text(text)) => {
                        ...
                        if let Err(e) =
                            handle_dashboard_message(user_id, &text, &db, &connection_manager).await
                        {
    ...
    connection_manager.remove_connection(connection_id).await;
    info!(
        "Dashboard user {} WebSocket connection fully closed",
        user_id
    );
}
...
async fn send_initial_dashboard_data(
    ...
    let agents = agent_service
        .list_agents_for_tenant(Uuid::new_v4())
        .await // TODO: Use actual tenant
    ...
    for agent in agents {
        ...
        connection_manager
            .send_to_user_dashboards(user_id, &message)
            .await
    ...
    for alert in recent_alerts.into_iter().take(10) {
        // Send last 10 alerts
...
async fn handle_dashboard_message(
    ...
    match request.get("type").and_then(|t| t.as_str()) {
        ...
        Some("refresh_data") => send_initial_dashboard_data(connection_manager, user_id, db).await,
        Some("ping") => {
            ...
            connection_manager
                .send_to_user_dashboards(user_id, &pong_message)
                .await
        }
        _ => {
            warn!(
                "Unknown dashboard message type from user {}: {:?}",
                user_id,
                request.get("type")
            );
            Ok(())
...
async fn handle_agent_command_request(
    ...
    let agent_id_str = request
        .get("agent_id")
        .and_then(|v| v.as_str())
        .ok_or("Missing agent_id in command request")?;

    let agent_id = Uuid::parse_str(agent_id_str).map_err(|e| format!("Invalid agent_id: {}", e))?;
    ...
    let command = threat_service
        .create_command(agent_id, user_id, command_request)
        .await
    ...
    info!(
        "User {} sent command {} to agent {}",
        user_id, command_type_for_log, agent_id
    );
    Ok(())
}
