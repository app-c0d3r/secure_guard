...
impl ConnectionManager {
    ...
    pub async fn add_agent_connection(
        ...
        info!(
            "Agent {} connected with connection {}",
            agent_id, connection_id
        );
        connection_id
    }

    pub async fn add_dashboard_connection(
        &self,
        user_id: Uuid,
        sender: mpsc::UnboundedSender<String>,
    ) -> Uuid {
        ...
        info!(
            "Dashboard user {} connected with connection {}",
    ...
    pub async fn send_to_all_dashboards(&self, message: &DashboardMessage) -> Result<(), String> {
                ...
                if let Some(connection) = connections.get(connection_id) {
                    match connection.sender.send(message_json.clone()) {
                        ...
                        Err(e) => {
                            error_count += 1;
                            warn!(
                                "Failed to send message to dashboard connection {}: {}",
                                connection_id, e
                            );
                        }
                    }
        ...
        info!(
            "Sent dashboard message to {} connections ({} errors)",
            sent_count, error_count
        );
        Ok(())
    }

    pub async fn send_to_user_dashboards(
        &self,
        user_id: Uuid,
            ...
            for connection_id in connection_ids {
                if let Some(connection) = connections.get(connection_id) {
                    if let Err(e) = connection.sender.send(message_json.clone()) {
                        warn!(
                            "Failed to send message to user {} dashboard {}: {}",
