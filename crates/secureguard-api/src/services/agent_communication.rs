...
use secureguard_shared::{
    Agent, CreateAgentRequest, AgentHeartbeat, AgentCommand, CommandStatus,
    CreateSecurityEventRequest, SystemMetrics, AgentStatus,
    SecureGuardError, Result, AgentMessage, DashboardMessage
};
...
impl AgentCommunicationService {
    ...
    pub async fn register_agent(
        ...
        info!("Registering new agent: {} ({})", request.agent_name, request.hostname);
        
        // Validate agent hardware fingerprint
        self.validate_hardware_fingerprint(&request.hardware_fingerprint).await?;
        ...
        self.broadcast_agent_status_update(agent.agent_id, AgentStatus::Online).await?;
    ...
    pub async fn process_heartbeat(
        ...
        self.agent_service.update_heartbeat(heartbeat_request).await?;
        ...
        // Update connection state
        {
            ...
            if let Some(connection) = connections.get_mut(&agent_id) {
                ...
                if let Some(metrics) = heartbeat.system_metrics {
                    ...
                    self.realtime_service.broadcast_system_metrics(agent_id, &metrics).await?;
                }
    ...
    pub async fn process_security_events(
        ...
        debug!("Processing {} security events from agent: {}", events.len(), agent_id);
        ...
        let events_with_agent: Vec<_> = events.into_iter()
            .map(|event| (agent_id, event))
            .collect();
            
        self.processing_pipeline.process_events_batch(events_with_agent).await?;
    ...
    pub async fn send_command_to_agent(
        ...
        let execution = CommandExecution {
            ...
            expected_completion: chrono::Utc::now() + chrono::Duration::from_std(self.command_timeout)
        ...
        self.message_router.route_agent_command(agent_id, &command).await
            .map_err(|e| SecureGuardError::ValidationError(format!("Failed to send command: {}", e)))?;
    ...
    pub async fn process_command_response(
        ...
        info!("Processing command response from agent {}: {} -> {:?}", 
        ...
        self.broadcast_command_status_update(command_id, status, result).await?;
    ...
    pub async fn get_communication_stats(&self) -> CommunicationStats {
        ...
        stats.online_agents = connections.values()
            .filter(|c| matches!(c.status, AgentStatus::Online))
            .count();
        stats.offline_agents = connections.values()
    ...
    pub async fn emergency_isolate_agent(&self, agent_id: Uuid, reason: String) -> Result<()> {
        warn!("EMERGENCY ISOLATION: Agent {} - Reason: {}", agent_id, reason);
        ...
        self.send_command_to_agent(agent_id, isolation_command).await?;
    ...
    pub async fn broadcast_emergency_command(&self, command_type: String, command_data: serde_json::Value) -> Result<Vec<Uuid>> {
        warn!("Broadcasting emergency command: {}", command_type);
        
        let connections = self.active_connections.read().await;
        let online_agents: Vec<_> = connections.iter()
        ...
        for agent_id in online_agents {
            ...
            match self.send_command_to_agent(agent_id, emergency_command).await {
                Ok(_) => successful_broadcasts.push(agent_id),
                Err(e) => error!("Failed to send emergency command to agent {}: {}", agent_id, e),
            }
        }
        
        info!("Emergency command broadcast complete: {}/{} agents", 
              successful_broadcasts.len(), connections.len());
    ...
    async fn broadcast_agent_status_update(&self, agent_id: Uuid, status: AgentStatus) -> Result<()> {
        self.realtime_service.update_agent_status(agent_id, status).await
    }
    ...
    async fn check_agent_heartbeats(&self) {
        ...
        let heartbeat_timeout = chrono::Duration::from_std(self.heartbeat_interval * self.max_missed_heartbeats)
        ...
        for agent_id in offline_agents {
            if let Err(e) = self.broadcast_agent_status_update(agent_id, AgentStatus::Offline).await {
                error!("Failed to broadcast agent offline status for {}: {}", agent_id, e);
            }
        }
    }
    ...
    async fn check_command_timeouts(&self) {
        ...
        for (command_id, execution) in tracking.iter_mut() {
            if now > execution.expected_completion && execution.retry_count < execution.max_retries {
                ...
                execution.expected_completion = now + chrono::Duration::from_std(self.command_timeout)
                    .unwrap_or_else(|_| chrono::Duration::seconds(300));
                
                warn!("Command {} timeout, retry {}/{}", 
                ...
                error!("Command {} permanently failed after {} retries", 
    ...
    async fn start_stats_collector(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                info!("Agent Communication Stats: {} total, {} online, {} offline, {} events/24h",
                      stats.total_agents, stats.online_agents, stats.offline_agents, stats.events_received_24h);
            }
        });
    }
